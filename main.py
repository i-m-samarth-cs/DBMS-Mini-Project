...
class Management:
    def __init__(self, root):
        self.window = root
        self.window.title("SCOE Student Management System")
        self.window.geometry("780x480")
        self.window.config(bg = "white")

        # Load the background image
        self.bg_image = Image.open("./Logo.jpeg")  # Ensure the image is in the same directory
        self.bg_image = self.bg_image.resize((780, 480), Image.Resampling.LANCZOS)  # Resize image to window size
